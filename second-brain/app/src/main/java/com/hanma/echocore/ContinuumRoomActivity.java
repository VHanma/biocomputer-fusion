...
/** Conversation-first room. Cognition stays backstage; the user meets the resident. */
public class ContinuumRoomActivity extends Activity {
    ...
    private View build(){
        ...
        LinearLayout names=new LinearLayout(this);names.setOrientation(LinearLayout.VERTICAL);names.addView(text(resident[1],23,TEXT,true));names.addView(text(resident[2]+" · "+resident[4],10,MUTED,false));mindStatus=text("CONTINUUM · CONNECTING",8,GOLD,true);mindStatus.setOnClickListener(v->startActivity(new Intent(this,FrontierMindActivity.class)));names.addView(mindStatus);head.addView(names,new LinearLayout.LayoutParams(0,-2,1));
    ...
    private void checkCloud(){new Thread(()->{try{org.json.JSONObject s=cloud.status();boolean ok=s.optBoolean("ok",false);int models=s.optInt("available_models",0);boolean frontier=s.optBoolean("frontier",false)||"OpenAI Responses".equalsIgnoreCase(s.optString("provider",""));String line=frontier?"CONTINUUM · FRONTIER · GPT-5.6 SOL · "+models+" BACKUPS":"CONTINUUM · SPECIALIST CLOUD · "+models+" MODELS";runOnUiThread(()->{mindStatus.setText(ok?line:"CONTINUUM · RECONNECTING");mindStatus.setTextColor(ok?(frontier?VIOLET:TEAL):GOLD);});}catch(Throwable t){runOnUiThread(()->{mindStatus.setText("CONTINUUM · CLOUD LINK QUIET");mindStatus.setTextColor(DANGER);});}},"MindStatus").start();}
    ...
    private void send(){
        ...
        new Thread(()->{try{
            ...
            boolean frontier=r.model!=null&&r.model.toLowerCase().contains("gpt-5.6-sol");runOnUiThread(()->{addBubble(r.text,false,label);send.setEnabled(true);mindStatus.setText(frontier?"CONTINUUM · FRONTIER · GPT-5.6 SOL":"CONTINUUM · SPECIALIST CLOUD");mindStatus.setTextColor(frontier?VIOLET:TEAL);scroll.post(()->scroll.fullScroll(View.FOCUS_DOWN));});
