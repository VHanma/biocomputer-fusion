...
/** Conversation-first room. Inbox-originated conversations retain their exact initiating thought. */
public class ContinuumRoomActivity extends Activity {
    ...
    private AscendantStore city;private String id;private String[] resident;private LinearLayout transcript;private ScrollView scroll;private EditText input;private Button send;private TextView mindStatus;private CloudMindClient cloud;private ThreadContinuityClient threads;
    private long inboxMessageId=0;private String inboxMessageText="",inboxMessageReason="",threadId="";

    @Override protected void onCreate(Bundle b){
        super.onCreate(b);requestWindowFeature(Window.FEATURE_NO_TITLE);id=getIntent().getStringExtra("resident_id");
        inboxMessageId=getIntent().getLongExtra("inbox_message_id",0);inboxMessageText=safe(getIntent().getStringExtra("inbox_message_text"));inboxMessageReason=safe(getIntent().getStringExtra("inbox_message_reason"));
        city=new AscendantStore(this);cloud=new CloudMindClient(this);threads=new ThreadContinuityClient(this);FoundationalArchive.install(city);ResidentExpansion.install(city);resident=city.resident(id);if(resident==null){finish();return;}
        getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(BG);setContentView(build());renderHistory();checkCloud();
    }
    ...
    private View build(){
        ...
        LinearLayout names=new LinearLayout(this);names.setOrientation(LinearLayout.VERTICAL);names.addView(text(resident[1],23,TEXT,true));names.addView(text(resident[2]+" · "+resident[4],10,MUTED,false));mindStatus=text(inboxMessageId>0?"CONTINUUM · REPLY THREAD":"CONTINUUM · CONNECTING",8,inboxMessageId>0?TEAL:GOLD,true);mindStatus.setOnClickListener(v->startActivity(new Intent(this,FrontierMindActivity.class)));names.addView(mindStatus);head.addView(names,new LinearLayout.LayoutParams(0,-2,1));
        ...
        LinearLayout composer=new LinearLayout(this);composer.setGravity(Gravity.BOTTOM|Gravity.CENTER_VERTICAL);input=new EditText(this);input.setTextColor(TEXT);input.setHintTextColor(0xFF657188);input.setHint(inboxMessageId>0?"Reply to "+resident[1]+"…":"Talk to "+resident[1]+"…");input.setTextSize(14);input.setMinLines(1);input.setMaxLines(5);input.setPadding(dp(13),dp(9),dp(13),dp(9));input.setBackground(round(0xFF101827,18));composer.addView(input,new LinearLayout.LayoutParams(0,-2,1));send=button(inboxMessageId>0?"REPLY":"SEND",VIOLET,0xFF05070C,10);LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(dp(76),dp(50));sp.setMargins(dp(8),0,0,0);composer.addView(send,sp);send.setOnClickListener(v->send());root.addView(composer);return root;
    }

    private void checkCloud(){new Thread(()->{try{org.json.JSONObject s=cloud.status();boolean ok=s.optBoolean("ok",false);int models=s.optInt("available_models",0);boolean frontier=s.optBoolean("frontier",false)||"OpenAI Responses".equalsIgnoreCase(s.optString("provider",""));String line=threadId.length()>0||inboxMessageId>0?"CONTINUUM · LIVE THREAD":frontier?"CONTINUUM · FRONTIER · GPT-5.6 SOL · "+models+" BACKUPS":"CONTINUUM · SPECIALIST CLOUD · "+models+" MODELS";runOnUiThread(()->{mindStatus.setText(ok?line:"CONTINUUM · RECONNECTING");mindStatus.setTextColor(ok?(threadId.length()>0||inboxMessageId>0?TEAL:frontier?VIOLET:TEAL):GOLD);});}catch(Throwable t){runOnUiThread(()->{mindStatus.setText("CONTINUUM · CLOUD LINK QUIET");mindStatus.setTextColor(DANGER);});}},"MindStatus").start();}

    private void renderHistory(){
        transcript.removeAllViews();List<String[]> mem=city.residentMemories(id,80);int shown=0;
        for(int i=mem.size()-1;i>=0;i--){String[]m=mem.get(i);if(m.length<3)continue;String type=m[2];if("USER".equals(type)){addBubble(m[1],true,"YOU");shown++;}else if("RESPONSE".equals(type)){String body=m[1],label=resident[1].toUpperCase();int cut=body.indexOf("\n::SPEAKER::");if(cut>=0){label=body.substring(cut+11).trim().toUpperCase();body=body.substring(0,cut).trim();}addBubble(body,false,label);shown++;}}
        if(shown==0&&inboxMessageId<=0)addSystem(openingLine());
        if(inboxMessageId>0&&!inboxMessageText.isEmpty()){addSystem(inboxMessageReason.isEmpty()?"They contacted you first.":"They contacted you first · "+inboxMessageReason);addBubble(inboxMessageText,false,resident[1].toUpperCase());}
    ...
    private void send(){
        final String q=input.getText().toString().trim();if(q.isEmpty())return;input.setText("");city.addResidentMemory(id,q,"USER",7,false);addBubble(q,true,"YOU");send.setEnabled(false);city.setResidentStatus(id,"WORKING");
        new Thread(()->{try{
            String modelInput=q;
            if(inboxMessageId>0||!threadId.isEmpty()){
                ThreadContinuityClient.ThreadState ts=threads.prepare(inboxMessageId,threadId,id,q);threadId=ts.threadId;inboxMessageId=0;
                modelInput="[CONTINUITY THREAD · internal context only · do not quote these labels]\n"+ts.context+"\n\nThe user's newest message in this same thread is:\n"+q;
            }
            CloudMindClient.Reply r=cloud.speak(id,modelInput);String speaker=r.speakerId==null||r.speakerId.isEmpty()?id:r.speakerId;String label=displayName(speaker).toUpperCase();
            if(!threadId.isEmpty())try{threads.recordReply(threadId,id,r.text);}catch(Throwable ignored){}
