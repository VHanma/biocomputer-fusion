...
public class CloudMindClient {
    ...
    public Reply speak(String residentId,String message) throws Exception{ensureRegistered();JSONObject b=new JSONObject().put("resident_id",residentId).put("message",message).put("depth",1);JSONObject r=postRaw(MIND+"/speak",b,true,180000);if(!r.optBoolean("ok",false))throw new Exception(r.optString("error","Cloud mind unavailable"));String text=r.optString("reply","").trim();if(text.isEmpty())throw new Exception("Cloud mind returned an empty reply");return new Reply(text,r.optString("speaker_id",residentId),r.optString("model",""),r.optString("routed_to",""));}
    public JSONObject council(String topic,int residents) throws Exception{ensureRegistered();JSONObject b=new JSONObject().put("topic",topic).put("residents",Math.max(3,Math.min(7,residents)));JSONObject r=postRaw(COUNCIL,b,true,260000);if(!r.optBoolean("ok",false))throw new Exception(r.optString("error","Cloud council unavailable"));if(r.optString("synthesis","").trim().isEmpty())throw new Exception("Cloud council returned no synthesis");return r;}
