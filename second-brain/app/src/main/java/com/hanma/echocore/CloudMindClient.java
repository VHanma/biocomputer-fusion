...
public class CloudMindClient {
    ...
    public synchronized void ensureRegistered() throws Exception{
        ...
        JSONObject body=new JSONObject().put("app_version","19.0.0").put("capabilities",new JSONArray().put("cloud_mind_v19").put("cloud_knowledge").put("resident_messages").put("approval_gate"));
    ...
    public Reply speak(String residentId,String message) throws Exception{ensureRegistered();JSONObject b=new JSONObject().put("resident_id",residentId).put("message",message).put("depth",1);return parseReply(postRaw(MIND+"/speak",b,true,180000),residentId);}
    public Reply askSource(String residentId,String sourceId,String sourceName,String question,boolean summarize)throws Exception{ensureRegistered();JSONObject b=new JSONObject().put("resident_id",residentId).put("source_id",sourceId).put("source_name",sourceName).put("question",question).put("summarize",summarize);return parseReply(postRaw(MIND+"/ask-source",b,true,180000),residentId);}
    ...
    private Reply parseReply(JSONObject r,String residentId)throws Exception{if(!r.optBoolean("ok",false))throw new Exception(r.optString("error","Cloud mind unavailable"));String text=r.optString("reply","").trim();if(text.isEmpty())throw new Exception("Cloud mind returned an empty reply");return new Reply(text,r.optString("speaker_id",residentId),r.optString("model",""),r.optString("routed_to",""));}
    private JSONObject postRaw(String url,JSONObject body,boolean auth,int readTimeout)throws Exception{HttpURLConnection c=(HttpURLConnection)new URL(url).openConnection();try{c.setRequestMethod("POST");c.setConnectTimeout(15000);c.setReadTimeout(readTimeout);c.setDoOutput(true);c.setRequestProperty("Content-Type","application/json");c.setRequestProperty("Accept","application/json");if(auth){c.setRequestProperty("X-Device-Id",prefs.get(KEY_ID,""));c.setRequestProperty("X-Device-Secret",prefs.getSecret(KEY_SECRET));}byte[] bytes=body.toString().getBytes(StandardCharsets.UTF_8);c.setFixedLengthStreamingMode(bytes.length);try(OutputStream o=c.getOutputStream()){o.write(bytes);}int code=c.getResponseCode();String raw=read(code>=200&&code<300?c.getInputStream():c.getErrorStream());if(code<200||code>=300)throw new Exception("Cloud HTTP "+code+": "+trim(raw,700));return raw.trim().isEmpty()?new JSONObject():new JSONObject(raw);}finally{c.disconnect();}}
